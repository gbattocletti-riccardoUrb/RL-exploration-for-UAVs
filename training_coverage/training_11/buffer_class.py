# buffer_class contains the functions to push-pull sampels from the experience buffer and to perform the learning steps
# i.e. to perform a gradient descent step (backprop + all the usual stuff)
...
class Buffer:
    def __init__(self, args):
        ...
        self.distance_goal_buffer = np.zeros((self.buffer_capacity, 2))
    ...
    def record(self, obs_tuple):
        ...
        self.distance_goal_buffer[index] = obs_tuple[4]

        self.buffer_counter += 1

    # Eager execution is turned on by default in TensorFlow 2. Decorating with tf.function allows TensorFlow to build a
    # static graph interpolated_output of the logic and computations in our function. This provides a large speed up for
    # blocks of code that contain many small TensorFlow operations such as this one.
    @tf.function
    def update(self, state_batch, action_batch, reward_batch, next_state_batch, actor_model, critic_model, target_actor, target_critic, gamma, goal_distance_batch):
        # Training and updating Actor & Critic networks.
        with tf.GradientTape() as tape:
            target_actions = target_actor([next_state_batch, goal_distance_batch], training=True)
        ...
        with tf.GradientTape() as tape:
            actions = actor_model([next_state_batch, goal_distance_batch], training=True)
    ...
    def learn(self, actor_model, critic_model, target_actor, target_critic, args):
        ...
        goal_distance_batch = tf.convert_to_tensor(self.distance_goal_buffer[batch_indices])

        # perform one backpropagation using the sampled minibatch - compute the loss and update parameters
        self.update(state_batch, action_batch, reward_batch, next_state_batch, actor_model, critic_model, target_actor, target_critic, args.gamma, goal_distance_batch)