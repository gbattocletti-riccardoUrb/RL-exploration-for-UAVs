...
def create_actor(args):
    ...
    out = layers.Conv2D(32, (7, 7), input_shape=(args.state_size, args.state_size, 1), strides=(3,3), activation="relu", padding='valid')(image_input)
    out = layers.MaxPooling2D(pool_size=(3, 3), strides=(2,2), padding='same')(out)
    ...
    out = layers.BatchNormalization()(out)
    out = layers.Conv2D(64, (3, 3), strides=(3, 3), activation="relu", padding='valid')(out)
    out = layers.MaxPooling2D(pool_size=(2, 2), strides=(2, 2), padding='same')(out)
    out = layers.BatchNormalization()(out)
    out = layers.Flatten()(out)
    image_out = layers.Dropout(.2)(out)

    goal_distance_input = layers.Input(shape=2)
    out = layers.Dense(64, activation="relu")(goal_distance_input)
...
def create_critic(args):
    ...
    out = layers.Conv2D(16, (7, 7), strides=(3, 3), input_shape=(args.state_size, args.state_size, 1), activation="relu")(state_input)
    out = layers.MaxPooling2D(pool_size=(3, 3), strides=(2,2), padding='same')(out)
    out = layers.BatchNormalization()(out)
