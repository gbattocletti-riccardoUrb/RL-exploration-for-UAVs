...
def draw_heatmap(heatmap_data, map_obj, ax):
    ...
    X = np.linspace(0.5, nx * map_obj.map_resolution, nx + 1, endpoint=True)
    Y = np.linspace(0.5, ny * map_obj.map_resolution, ny + 1, endpoint=True)
    # plt.register_cmap(cmap=plt.cm.winter)
    # plt.set_cmap(plt.cm.winter)
    plt_color_map = ax.pcolormesh(X, Y, heatmap_data.T, shading='auto', zorder=0)
