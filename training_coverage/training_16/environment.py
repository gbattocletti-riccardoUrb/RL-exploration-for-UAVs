...
class Environment:
    ...
    def __init__(self):
        ...
        self.reached_goal_distance = 0.3
    ...
    def compute_reward(self, drone, cumulative_explored_cells):
        if self.obstacle_map[drone.index_position[0]][drone.index_position[1]] == 1:
            r = - 5
        elif self.last_observation[drone.drone_ID, drone.pos2index(drone.goal[0]), drone.pos2index(drone.goal[0])] > 0:
            r = - 1
        else:
            r = 0
            if cumulative_explored_cells[drone.drone_ID] - self.prev_explored_cells[drone.drone_ID] >= 200:
                r += (cumulative_explored_cells[drone.drone_ID] - self.prev_explored_cells[drone.drone_ID]) / 600
                # r += 1
            else:
                r += 0
    ...
    def visualize(self, episode_number, show_flag, save_flag, figure_path, drone_list, reward):
        ...
        heatmap.draw_heatmap(drone_list[0].obstacle_map, self, ax0)
        # p_map = heatmap.draw_heatmap(self.last_observation[0, :, :], self, ax0)
        ...
        heatmap.draw_heatmap(drone_list[1].obstacle_map, self, ax1)
        #
        ...
        heatmap.draw_heatmap(drone_list[2].obstacle_map, self, ax2)
        ...
        #ax3.plot(drone_list[3].goal[0], drone_list[3].goal[1], 'bx')
        #ax3.plot(drone_list[3].position_history[0, 0], drone_list[3].position_history[0, 1], 'r.', markersize=3)
        #ax3.plot(drone_list[3].goal_history[:, 0], drone_list[3].goal_history[:, 1], 'rx', markersize=3)
        # heatmap.draw_heatmap(self.last_observation[3, :, :], self, ax3)
        #heatmap.draw_heatmap(drone_list[3].covered_area_matrix, self, ax3)
