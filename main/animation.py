...
def plot_figures(drone_list):
    ...
    # plt.savefig('simulation_images/coverage_sim_graph1.png', dpi=250)
    plt.tight_layout()
    plot_figures2(drone_list)

def plot_figures2(drone_list):
    data_length = len(drone_list[0].single_coverage[2: -1])
    single_coverage = np.zeros([len(drone_list), data_length])
    for drone in drone_list:
        single_coverage[drone.drone_ID-1, :] = np.array(drone.single_coverage[2: -1])

    plt.rc('font', family='serif')
    dpi = 150
    figure_size_pixel = 1500, 1000
    figure_size = figure_size_pixel[0] / dpi, figure_size_pixel[1] / dpi
    final_figure2 = plt.figure(4, dpi=dpi, figsize=figure_size)
    ax = final_figure2.add_subplot(111, autoscale_on=False, ylim=[0, 100], xlim=[0, data_length])
    ax.set_xlabel('step number', labelpad=8)
    ax.set_ylabel('% of' + '\n' + 'covered area')
    for ii in range(len(drone_list)):
        ax.plot(np.linspace(0, data_length - 1, data_length), single_coverage[ii])
    # plt.savefig('simulation_images/coverage_sim_graph2.png', dpi=250)
    plt.tight_layout()
    plt.show()
    save_csv_data(drone_list, single_coverage)
    exit(0)

def save_csv_data(drone_list, single_coverage):
    drones_trajectories = np.zeros([len(drone_list[0].position_history), len(drone_list)*2])
    # goal_vector_length = np.max([len(drone_list[0].goal_history), len(drone_list[1].goal_history), len(drone_list[2].goal_history), len(drone_list[3].goal_history)])
    # goals_history = np.zeros([goal_vector_length, len(drone_list)*2])
    # goals_history[:] = np.nan

    jj = 0
    for ii in range(len(drone_list)):
        drones_trajectories[:, jj:jj+2] = drone_list[ii].position_history
        # goals_history[:len(drone_list[ii].goal_history), jj:jj+2] = drone_list[ii].goal_history
        jj += 2

    np.savetxt('single_coverage.csv', np.round(single_coverage.T, 2))
    np.savetxt('drones_trajectories.csv', np.round(drones_trajectories, 2))
    # np.savetxt('goals_history.csv', np.round(goals_history, 2))
...
def animate(drone_list, drone_selection_index, world, media_file_name, duration, save_animation, begin_time):
    ...
    def init_plot(map_object):
        ...
        full_obstacles, = ax.plot([], [], 's', color='grey', ms=2.5, zorder=4, alpha=0.1)
        ...
        # texts
        text_step_counter = ax.text(0.05, 0.95, '', fontsize=8, transform=ax.transAxes, bbox={'facecolor':'w', 'alpha':1, 'pad':5})
    ...
    def init_plot_coverage(map_object):
        ...
        drone_0_coverage, = ax.plot([], [], 'ro', ms=6, zorder=21)
        drone_1_coverage, = ax.plot([], [], 'go', ms=6, zorder=21)
        drone_2_coverage, = ax.plot([], [], 'bo', ms=6, zorder=21)
        drone_3_coverage, = ax.plot([], [], 'mo', ms=6, zorder=21)

        objective_0_coverage, = ax.plot([], [], 'rx', ms=6, zorder=16)
        objective_1_coverage, = ax.plot([], [], 'gx', ms=6, zorder=16)
        objective_2_coverage, = ax.plot([], [], 'bx', ms=6, zorder=16)
        objective_3_coverage, = ax.plot([], [], 'mx', ms=6, zorder=16)

        # texts
        coverage_percentage = ax.text(0.05, 0.95, '', fontsize=8, transform=ax.transAxes, bbox={'facecolor':'w', 'alpha':1, 'pad':5})

        return ax, ms, drones_coverage, obstacles_coverage, trajectory_coverage, coverage_percentage, k_means_center, drone_0_coverage, drone_1_coverage, drone_2_coverage, drone_3_coverage, \
               objective_0_coverage, objective_1_coverage, objective_2_coverage, objective_3_coverage,
    ...
    def init_animation():
        ...
        drone_2.set_data([], [])
        drone_3.set_data([], [])
        drone_0_coverage.set_data([], [])
        drone_1_coverage.set_data([], [])
        drone_2_coverage.set_data([], [])
        drone_3_coverage.set_data([], [])
        ...
        objective_0_coverage.set_data([], [])
        objective_1_coverage.set_data([], [])
        objective_2_coverage.set_data([], [])
        objective_3_coverage.set_data([], [])
        k_means_center.set_data([], [])

        return drones, objectives, obstacles, full_obstacles, local_minima, trajectory, text_step_counter, drones_coverage,\
               ...
               objective_0, objective_1, objective_2, objective_3, k_means_center, drone_0_coverage, drone_1_coverage, drone_2_coverage, drone_3_coverage, \
               objective_0_coverage, objective_1_coverage, objective_2_coverage, objective_3_coverage,

    def main_animation(iteration):
        """ Main cicle of animation"""
        main.make_action_cycle(drone_list, world)
        ...
        drones_coverage.set_markersize(ms*1.5)
        ...
        trajectory_0.set_data(drone_list[0].smoothed_trajectory_points[x][:], drone_list[0].smoothed_trajectory_points[y][:])
        trajectory_1.set_data(drone_list[1].smoothed_trajectory_points[x][:], drone_list[1].smoothed_trajectory_points[y][:])
        trajectory_2.set_data(drone_list[2].smoothed_trajectory_points[x][:], drone_list[2].smoothed_trajectory_points[y][:])
        trajectory_3.set_data(drone_list[3].smoothed_trajectory_points[x][:], drone_list[3].smoothed_trajectory_points[y][:])

        drone_0.set_data(all_positions[0, x], all_positions[0, y])
        drone_0.set_markersize(ms*1.7)
        drone_1.set_data(all_positions[1, x], all_positions[1, y])
        drone_1.set_markersize(ms*1.7)
        drone_2.set_data(all_positions[2, x], all_positions[2, y])
        drone_2.set_markersize(ms*1.7)
        drone_3.set_data(all_positions[3, x], all_positions[3, y])
        drone_3.set_markersize(ms*1.7)

        drone_0_coverage.set_data(all_positions[0, x], all_positions[0, y])
        drone_0_coverage.set_markersize(ms*1.7)
        drone_1_coverage.set_data(all_positions[1, x], all_positions[1, y])
        drone_1_coverage.set_markersize(ms*1.7)
        drone_2_coverage.set_data(all_positions[2, x], all_positions[2, y])
        drone_2_coverage.set_markersize(ms*1.7)
        drone_3_coverage.set_data(all_positions[3, x], all_positions[3, y])
        drone_3_coverage.set_markersize(ms*1.7)

        objective_0.set_data(all_goal_positions[0, x], all_goal_positions[0, y])
        objective_0.set_markersize(ms*1.7)
        objective_1.set_data(all_goal_positions[1, x], all_goal_positions[1, y])
        objective_1.set_markersize(ms*1.7)
        objective_2.set_data(all_goal_positions[2, x], all_goal_positions[2, y])
        objective_2.set_markersize(ms*1.7)
        objective_3.set_data(all_goal_positions[3, x], all_goal_positions[3, y])
        objective_3.set_markersize(ms*1.5)

        objective_0_coverage.set_data(all_goal_positions[0, x], all_goal_positions[0, y])
        objective_0_coverage.set_markersize(ms*1.7)
        objective_1_coverage.set_data(all_goal_positions[1, x], all_goal_positions[1, y])
        objective_1_coverage.set_markersize(ms*1.7)
        objective_2_coverage.set_data(all_goal_positions[2, x], all_goal_positions[2, y])
        objective_2_coverage.set_markersize(ms*1.7)
        objective_3_coverage.set_data(all_goal_positions[3, x], all_goal_positions[3, y])
        objective_3_coverage.set_markersize(ms*1.5)
        # local_minima.set_data(local_minima_list[:, x], local_minima_list[:, y])
        # local_minima.set_markersize(ms*5)
        # k_means_center.set_data(world.kmeans_center[:, 0], world.kmeans_center[:, 1])
        ...
        #     plt.savefig('simulation_images/coverage_sim' + str(int(iteration/save_freq)) + '.png', dpi=250)
            # for drone in drone_list:
            #     np.savetxt('observation' + str(int(iteration/50)) + '_drone' + str(drone.drone_ID) + '.csv', np.round(drone.last_observation, 2))
        ...
        return drones, objectives, selected_drone, selected_objective, obstacles, full_obstacles, local_minima, plt_color_map,\
    ...
    (ax_2, ms, drones_coverage, obstacles_coverage, trajectory_coverage, coverage_percentage, k_means_center, drone_0_coverage, drone_1_coverage, drone_2_coverage, drone_3_coverage, objective_0_coverage, objective_1_coverage, objective_2_coverage, objective_3_coverage,) = init_plot_coverage(world)
