...
class Drone:
    def __init__(self):
        ...
        self.min_goal_distance = 0.5                            # min distance to reach the goal
    ...
    def generate_border_obstacles(self, world):
        ...
        self.newfound_obstacle_list = np.append(world.obstacle_list, border_obstacles, axis=0)
    ...
    def reinforcement_learning_path_planning(self, sim_step):
        x = 0
        y = 1
        goal_distance_x = self.goal[x] - self.position[x]
        goal_distance_y = self.goal[y] - self.position[y]
    ...
    def coverage_voronoi(self, drone_list):
        ...
        other_drone_kmeans_center = np.empty([0, 2])
        unknown_points = np.vstack((self.y_cohordinate_matrix[self.covered_area_matrix==0], self.x_cohordinate_matrix[self.covered_area_matrix==0])).T
        kmeans = KMeans(n_clusters=5, random_state=0, init='k-means++', n_init=10).fit(unknown_points)
        exploration_area_centers = kmeans.cluster_centers_
        for drone in drone_list:
            if drone.drone_ID != self.drone_ID:
                other_drone_kmeans_center = np.vstack([other_drone_kmeans_center, drone.kmeans_center])
        for new_goal_position in exploration_area_centers:
            if (np.where((np.round(other_drone_kmeans_center) == np.round(new_goal_position)).all(axis=1))[0]).size > 0:
                pass
            else:
                if self.kmeans_center.size == 0  and self.covered_area_matrix[self.pos2index(new_goal_position[0]), self.pos2index(new_goal_position[1])] == 0:     # if vector is empty and area is not explored
                    new_goals = np.vstack([new_goals, new_goal_position])
                elif (np.where((np.round(self.kmeans_center) == np.round(new_goal_position)).all(axis=1))[0]).size > 0  and self.covered_area_matrix[self.pos2index(new_goal_position[0]), self.pos2index(new_goal_position[1])] == 0:    # check if the goal is already present and the area is not explored
                    new_goals = np.vstack([new_goals, new_goal_position])

        self.kmeans_center = np.vstack([self.kmeans_center, new_goals])         # add the new goals to the possible goal
        # for goal_position in self.kmeans_center:                                # delete the already explored position goal
        #     if self.covered_area_matrix[self.pos2index(goal_position[0]), self.pos2index(goal_position[1])] != 0:
        ...
        x_distances = np.ones(len(self.kmeans_center))*self.position[0] - np.array([self.kmeans_center[:, 0]])
        y_distances = np.ones(len(self.kmeans_center))*self.position[1] - np.array([self.kmeans_center[:, 1]])
        distances = np.hypot(x_distances, y_distances)[0]
        goal_idx = np.where(distances == np.min(distances))[0][0]
        self.old_goal = self.goal
        self.goal = self.kmeans_center[goal_idx, :]
        # goal_idx = np.where((self.kmeans_center == self.goal).all(axis=1))[0]
        self.kmeans_center = np.delete(self.kmeans_center, goal_idx, axis=0)

        # communication of the share list and command to delete already taken goals
        for drone in drone_list:
            if drone.drone_ID != self.drone_ID:
                goal_idx = np.where((np.round(drone.kmeans_center) == np.round(self.goal)).all(axis=1))[0]
                drone.kmeans_center = np.delete(drone.kmeans_center, goal_idx, axis=0)

                for goal_position in self.kmeans_center:
                    if (np.where((np.round(drone.kmeans_center) == np.round(goal_position)).all(axis=1))[0]).size == 0:
                        drone.kmeans_center = np.vstack([drone.kmeans_center, goal_position])
    ...
    def coverage_RL(self):
        ...
        state = tf.expand_dims(tf.convert_to_tensor(state), 0)
        drone_position = (np.array(self.position)) / [self.map_dimension_x, self.map_dimension_y]
        ...
        new_goal[0] = np.clip(new_goal[0], 0, self.map_dimension_x - 1)
        new_goal[1] = np.clip(new_goal[1], 0, self.map_dimension_y - 1)
    ...
    def oscillation_detection(self):
        if len(self.position_history) > 10:
            ...
            if len(unique_positions) < 8:                           # if the number of repeated position is higher than a treshold the oscillation flag is set true
                return True
            else:
                return False
        else:
    ...
    def interpolate_trajectory(self):
        ...
        interpolated_trajectory = interpolate.splev(u3, tck)
        self.smoothed_trajectory_points = interpolated_trajectory
        # self.smoothed_trajectory_points = [control_points_x, control_points_y]
        return
    ...
    def action(self, drone_list, sim_step):
        ...
        #     self.coverage_RL()
            ...
            if self.local_minima_flag:
                ...
                self.reinforcement_learning_path_planning(sim_step)


    def action2(self, drone_list, sim_step):
        # if not self.goal_updated:
